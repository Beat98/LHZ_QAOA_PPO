...
parser.add_argument('--n_max_gates', default=3, type=int, help="maximal sequence length")
parser.add_argument('--n_steps', default=50, type=int, help="steps between policy updates")
parser.add_argument('--n_timesteps', default=3000, type=int, help="number of environment steps")
parser.add_argument('--num_executions', default=10, type=int, help="number of experiments")
parser.add_argument('--gamma', default=0.99, type=float, help="")
parser.add_argument('--qaoa_optimizer', default="SLSQP", type=str, help="")
parser.add_argument('--reps', default=10, type=int, help="")
parser.add_argument('--num_qubits', default=10, type=int, help="")
...
parser.add_argument('--commutator', default="both", type=str, help="")
...
if commutator == "both":
    actions = ['X', 'Z', 'C', 'A', 'B']
elif commutator == "second_order_v2":
...
# experiment_name = os.path.join(f"num_executions_{num_executions}_reps_{reps}",
#                                f"n_steps_{n_steps}")
experiment_name = os.path.join(f"commutators_{commutator}", f"num_qubits_{num_qubits}_n_max_gates_{n_max_gates}",
                               f"local_fields_{''.join([str(f) for f in local_fields])}_num_executions_{num_executions}_reps_{reps}")
...
for i in range(num_executions):
    ...
    env = QaoaEnv(problem_dict=problem_dict, reps=reps,
                  n_max_gates=n_max_gates, env_setup=env_setup, qaoa_optimizer=qaoa_optimizer,
                  final_decoding=final_decoding,
